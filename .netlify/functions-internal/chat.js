...
function getResponse(message) {
  ...
  if (msg.includes('comprar') || msg.includes('donde') || msg.includes('como adquirir')) return mockResponses.comprar;
  if (msg.includes('arcadiab') || msg.includes('arcadia') || msg.includes('b2x')) return mockResponses.arcadiab;
  if (msg.includes('prestamo') || msg.includes('préstamo') || msg.includes('liquidez') || msg.includes('colateral')) return mockResponses.prestamo;
  if (msg.includes('curso') || msg.includes('capitulo') || msg.includes('capítulo')) return mockResponses.curso;
  if (msg.includes('inflacion') || msg.includes('inflación') || msg.includes('precios')) return mockResponses.inflacion;
  if (msg.includes('oro') && !msg.includes('tesoro')) return mockResponses.oro;
  if (msg.includes('peso') && (msg.includes('mexicano') || msg.includes('evalua') || msg.includes('bateria'))) return mockResponses.peso;
  if (msg.includes('mexico') || msg.includes('méxico') || msg.includes('mexicano')) return mockResponses.mexico;
  if (msg.includes('que es bitcoin') || msg.includes('qué es bitcoin') || msg.includes('explica bitcoin')) return mockResponses.bitcoin;
  if (msg.includes('ayuda') || msg.includes('ayudame') || msg.includes('ayúdame')) return mockResponses.ayuda;
...
exports.handler = async (event) => {
  ...
  try {
    ...
    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({ response: getResponse(message), timestamp: new Date().toISOString() })
    };
  } catch (error) {
    return {
      statusCode: 500,
      headers,
      body: JSON.stringify({ error: 'Error', response: 'Lo siento, tuve un problema técnico. Intenta de nuevo o contacta hola@arcadiab.com' })
    };
  }
};
